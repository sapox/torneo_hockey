class Arbitro < ActiveRecord::Base
  ...
  presence: {:message => "no deberia estar en blanco"}
  validates :nombre, :apellido, :format => { :with => /^[a-zA-Z ]+$/, :message => "Solo letras estan permitidas"}
  validates :dni, :telefono_celular, :telefono_fijo, :numericality => { :only_integer => true , :message => "Solo numeros estan permitidos"}
end
