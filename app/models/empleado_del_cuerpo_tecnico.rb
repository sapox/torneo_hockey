class EmpleadoDelCuerpoTecnico < ActiveRecord::Base
  ...
  validates :apellido, :direccion, :dni, :email, :fecha_de_nacimiento, :nombre, :telefono_celular, :telefono_fijo, presence: {:message => "no deberia estar en blanco"}
  validates :nombre, :apellido, :direccion, :format => { :with => /^[a-zA-Z ]+$/, :message => "Solo letras estan permitidas"}
  validates :dni, :telefono_celular, :telefono_fijo, :numericality => { :only_integer => true, :message => "Solo numeros permitidos" }
end
